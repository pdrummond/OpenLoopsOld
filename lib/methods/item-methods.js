...
Meteor.methods({
	...
	updateItemLabels: function(itemId, newLabels) { 
		...
		oldLabelNames = [];
		_.each(oldLabels, function(labelId) {
			oldLabelNames.push(Labels.findOne(labelId).title);
		});
		newLabelNames = [];
		_.each(newLabels, function(labelId) {
			newLabelNames.push(Labels.findOne(labelId).title);
		});
		...
		Meteor.call('createActivity', {
			...
			item: item,
			attr:'labels',
			itemOldAttr: oldLabelNames.join(","),
			itemNewAttr: newLabelNames.join(","),
			boardId: item.boardId
		});
	},
